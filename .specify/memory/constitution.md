...
- Added Sections:
  - Hospital Websites & Healthcare Platforms (comprehensive healthcare requirements)
    - Multi-hospital/multi-tenant architecture for configurable deployments
    - Healthcare-specific features (appointments, doctors, departments, services)
    - Patient data privacy & compliance (HIPAA, HITECH, GDPR, local regulations)
    - Integration requirements (HMS, EHR, payment gateways, communication APIs)
    - Trust-building & credibility features (certifications, testimonials)
    - Emergency contact systems and critical information display
    - SEO & performance optimizations for healthcare context
    - Mobile-first design with accessibility for diverse patient populations
    - Multi-language support for healthcare accessibility
    - AI-powered enhancements for healthcare platforms
    - Recommended tech stacks for hospital websites (Next.js, Django, FastAPI options)
    - Database schema design for multi-tenant hospital platforms
    - Security requirements for handling sensitive medical information
...
- Templates Requiring Updates:
  ✅ .specify/templates/plan-template.md (Add hospital website planning considerations)
  ✅ .specify/templates/spec-template.md (Add hospital-specific requirements sections)
  ✅ .specify/templates/tasks-template.md (Add hospital platform implementation tasks)
  ⚠️ Need to create: hospital-website-checklist.md, hipaa-compliance-checklist.md, healthcare-multi-tenant-testing-checklist.md
  ⚠️ Previously flagged: school-website-checklist.md, ferpa-compliance-checklist.md, multi-tenant-testing-checklist.md
- Follow-up TODOs:
  - Create hospital website launch checklist
  - Create HIPAA/HITECH compliance verification checklist
  - Create healthcare multi-tenant security testing guide
  - Add hospital database schema template
  - Add healthcare role-based access control (RBAC) implementation guide
  - Add patient data privacy policy template
  - Add hospital website user guides (admin, doctor, patient roles)
  - Add hospital-HMS integration documentation
  - Previously flagged (schools): Create school website launch checklist, FERPA/COPPA compliance verification checklist, multi-tenant security testing guide, school database schema template, RBAC implementation guide, student data privacy policy template, school website user guides
-->
...
### Hospital Websites & Healthcare Platforms

**Additional Requirements for Healthcare Institutions:**

Hospital websites serve patients, medical staff, and administrators while promoting healthcare services and facilitating patient acquisition. They require configurable multi-hospital architecture, strict compliance with healthcare privacy regulations, trust-building features, and seamless integration with Hospital Management Systems (HMS).

**Core Healthcare Features:**

- Multi-hospital support with single codebase and per-hospital configuration
- Hospital-specific branding (name, logo, colors, domain/subdomain)
- Professional homepage with trust-building elements
- Doctor profiles and listings
- Department and specialty showcases
- Services and treatment descriptions
- Facilities and equipment information
- Online appointment booking system
- OPD schedule display
- Patient portal (login/registration for HMS integration)
- Emergency contact information (prominent display)
- Location and branch information with maps
- Contact forms with anti-spam protection
- News, blog, and announcements section
- Careers and job opportunities
- About hospital and mission/vision
- Patient testimonials and success stories
- Certifications, accreditations, and partner logos
- Health packages and preventive care programs
- Insurance and payment information
- Ambulance and emergency services information
- Online reports and test results access (HMS integration)
- Health information library and FAQs
- Event and health camp information
- Corporate wellness programs
- International patient services (if applicable)

**Multi-Hospital Architecture Requirements:**

Hospital websites MUST support deployment for multiple hospitals from a single codebase with minimal configuration changes.

**Configuration-Based Deployment:**

- MUST use configuration file per hospital (JSON, YAML, or environment variables)
- MUST support hospital-specific branding without code changes:
  - Hospital name (e.g., "ABC General Hospital" → "XYZ Specialty Clinic")
  - Logo and favicon
...
- MUST support hospital-specific content configuration:
  - Departments and specialties offered
  - Services and treatments available
  - Facilities and equipment
  - Doctor profiles and availability
  - Operating hours and OPD schedules
  - Emergency contact numbers
  - Location coordinates for maps
- MUST implement theme system for color customization
- MUST support feature flags per hospital (enable/disable modules)
- MUST allow configuration-based content overrides
- SHOULD support multi-language configuration per hospital region
- SHOULD support currency configuration for pricing (if displayed)
...
hospital:
  ...
  logo: "/assets/abc-hospital-logo.png"
  favicon: "/assets/abc-favicon.ico"
...
integrations:
  hms_api_url: "https://hms.abchospital.com/api"
  payment_gateway: "stripe"
  sms_provider: "twilio"
  email_provider: "sendgrid"
```

**Multi-Tenant vs Configuration Approach:**

This constitution recommends **configuration-based multi-hospital support** rather than full multi-tenancy:

- **Configuration Approach (Recommended for Hospital Websites)**:
  - Each hospital deployment is separate (separate domain, separate database, separate deployment)
  - Hospitals share codebase but NOT infrastructure or data
  - Configuration file customizes branding, content, integrations
  - Simpler security model (no cross-hospital data leakage risk)
  - Easier compliance with healthcare regulations (data isolation by default)
  - Better for selling/licensing website to individual hospitals
  - Deploy once per hospital with hospital-specific config

- **Multi-Tenant Approach (Optional, for Platform Providers)**:
  - Multiple hospitals share single database and infrastructure
  - MUST implement strict row-level security (tenant_id/hospital_id on all tables)
  - MUST implement tenant-aware authentication and authorization
  - MUST test tenant data isolation rigorously
  - More complex but better for SaaS platform serving many hospitals
  - Requires robust compliance measures and audit trails

**Choose configuration-based for**: Individual hospital websites, hospital website template products
**Choose multi-tenant for**: Healthcare platform SaaS products serving 100+ hospitals

**Patient Data Privacy & Compliance (CRITICAL):**

Hospital websites handle sensitive health information and MUST comply with strict regulations:

**HIPAA Compliance (US - Health Insurance Portability and Accountability Act):**

- MUST implement HIPAA-compliant data handling if collecting Protected Health Information (PHI)
- PHI includes: names, addresses, dates (birth, admission, discharge, death), phone numbers, fax numbers, email addresses, Social Security numbers, medical record numbers, health plan beneficiary numbers, account numbers, certificate/license numbers, vehicle identifiers, device identifiers/serial numbers, web URLs, IP addresses, biometric identifiers, full-face photos, and any other unique identifying number/code
- MUST encrypt PHI at rest (AES-256) and in transit (TLS 1.3+)
- MUST implement access controls (role-based access)
- MUST maintain audit logs of all PHI access (who, what, when, where)
- MUST implement automatic session timeout (15 minutes for PHI access)
- MUST conduct regular risk assessments
- MUST implement data breach notification procedures (within 60 days)
- MUST sign Business Associate Agreements (BAA) with third-party services handling PHI
- MUST train staff on HIPAA requirements annually
- MUST implement secure password policies (minimum 12 characters, complexity, rotation)
- MUST implement multi-factor authentication (MFA) for staff accessing PHI
- MUST implement data backup and disaster recovery procedures
- MUST sanitize/de-identify PHI before using for analytics or testing
- MUST provide patients right to access their PHI
- MUST provide patients right to request corrections
- MUST provide patients right to request restrictions on uses/disclosures
- MUST obtain patient consent before collecting PHI beyond minimum necessary

**HITECH Act Compliance (US - Health Information Technology for Economic and Clinical Health):**

- Extends HIPAA with stricter enforcement and breach notification requirements
- MUST report breaches affecting 500+ individuals to HHS and media
- MUST report breaches affecting <500 individuals to HHS annually
- MUST implement audit controls and monitoring
- MUST implement encryption for PHI (not technically required but creates safe harbor)

**GDPR Compliance (EU - General Data Protection Regulation):**

- MUST obtain explicit consent for health data processing
- MUST provide right to access personal data
- MUST provide right to rectification (correct inaccurate data)
- MUST provide right to erasure ("right to be forgotten")
- MUST provide right to data portability
- MUST implement data minimization (collect only necessary data)
- MUST implement privacy by design and by default
- MUST maintain records of processing activities
- MUST implement data breach notification (within 72 hours)
- MUST appoint Data Protection Officer (DPO) if processing health data at scale
- MUST conduct Data Protection Impact Assessments (DPIA) for health data processing
- Health data is "special category" requiring heightened protection

**General Healthcare Data Protection Best Practices:**

- MUST encrypt all patient PII and PHI at rest and in transit
- MUST implement HTTPS with HSTS headers (Strict-Transport-Security)
- MUST implement Content Security Policy (CSP) headers
- MUST sanitize all user inputs to prevent injection attacks
- MUST implement rate limiting to prevent brute force attacks
- MUST implement secure password reset process (not via email for PHI access)
- MUST implement account lockout after failed login attempts
- MUST disable accounts for inactive users (90 days)
- MUST conduct regular security audits and penetration testing
- MUST implement intrusion detection and prevention
- MUST implement DDoS protection
- MUST conduct background checks for staff with PHI access
- MUST provide transparent privacy policy in plain language
- MUST provide Notice of Privacy Practices (HIPAA requirement)
- MUST implement patient consent management
- MUST provide opt-out mechanisms for marketing communications
- MUST NOT use patient data for marketing without explicit consent
- MUST implement secure file upload with virus scanning
- MUST log all access to patient records (audit trail)

**What Constitutes PHI That MUST Be Protected:**
...
- Diagnosis, treatment, and medical history information
- Prescription and medication information
- Lab results and test results
- Insurance information
- Payment and billing information
- Photos and videos of patients
- IP addresses and device identifiers (if linked to patient)
- Any other information that can identify a patient

**Public Website vs Patient Portal Distinction:**

- **Public Website**: No PHI, minimal PII, no HIPAA compliance required
  - General hospital information (services, doctors, departments)
  - Contact information and location
  - Educational content and blogs
  - Appointment request forms (name, phone, preferred date - NOT PHI until confirmed)
  - Contact forms for general inquiries

- **Patient Portal** (HMS Integration): Contains PHI, FULL HIPAA compliance required
...
**Recommendation**: Keep public website and patient portal SEPARATE with different authentication and compliance requirements. Link to patient portal from public website but maintain separate infrastructure.

**Appointment Booking System:**

- Patient can browse doctors by department or specialty
- Patient can view doctor profile (photo, qualification, experience, specialties, OPD timings)
- Patient can select doctor and preferred date/time slot
- Patient provides basic details (name, phone, email, reason for visit)
- System validates slot availability (real-time check with HMS if integrated)
- System sends confirmation via SMS and email
- System creates appointment record in HMS (if integrated) or stores for manual processing
- Patient receives appointment ID and instructions
- MUST implement spam protection (reCAPTCHA v3 or similar)
- MUST implement rate limiting to prevent appointment spam
- MUST validate phone and email format
- SHOULD implement OTP verification for phone number
- SHOULD implement appointment reminders (24 hours before, 2 hours before)
- SHOULD allow patients to cancel/reschedule appointments
- SHOULD provide waitlist functionality for fully booked slots

**HMS (Hospital Management System) Integration:**

- MUST provide API integration documentation
- MUST support common HMS platforms or custom API
- Integration points:
  - Doctor information sync (profiles, specialties, OPD schedules)
  - Department and service information sync
  - Appointment booking (create, read, update, cancel)
  - Appointment slot availability (real-time check)
  - Patient authentication (login to patient portal)
  - Medical records retrieval (if patient portal integrated)
  - Lab results retrieval (if patient portal integrated)
  - Billing and payment information (if online payment integrated)
- MUST implement secure API authentication (OAuth 2.0, API keys with rotation)
- MUST implement API rate limiting
- MUST implement error handling and fallback mechanisms
- MUST implement webhook support for real-time updates (appointment confirmations, cancellations)
- MUST log all API calls for audit and debugging
- MUST implement data validation for API responses
- SHOULD implement API versioning for backward compatibility
- SHOULD implement caching for frequently accessed data (doctor profiles, departments)

**Doctor Profiles Module:**

- Doctor listing page with search and filters
  - Filter by department/specialty
  - Filter by availability (OPD days)
  - Search by doctor name
  - Sort by experience, ratings (if applicable)
- Doctor profile page (individual):
  - Professional photo
  - Full name and designation
  - Qualification and degrees
  - Specialties and areas of expertise
  - Years of experience
  - OPD schedule (days and timings)
  - Consultation fee (optional, configurable)
  - Languages spoken (optional)
  - Awards and recognitions (optional)
  - Patient reviews and ratings (optional, if enabled)
  - "Book Appointment" call-to-action button
- MUST implement image optimization for doctor photos (WebP with fallback)
- MUST implement lazy loading for doctor images
- MUST support doctor profile management via CMS or HMS sync
- SHOULD implement doctor availability calendar view
- SHOULD support multiple OPD locations for doctors practicing at multiple branches

**Departments & Services Module:**
...
- Individual department pages with:
  ...
  - Team of doctors in department
  - Success stories and case studies (optional)
  - Related health articles (optional)
  - "Book Appointment" or "Contact Us" CTA
- Services listing page
- Individual service/treatment pages with:
  - Service name and detailed description
  - Benefits and outcomes
  - Treatment procedure overview
  - Recommended for (conditions/symptoms)
  - Related departments and doctors
  - FAQs
  - "Book Appointment" CTA
- MUST implement SEO optimization for department and service pages
- MUST implement schema markup (MedicalSpecialty, MedicalProcedure)
- SHOULD implement related content recommendations

**Homepage Design & Sections:**

- **Hero Section**:
  - Trust-building headline (e.g., "Your Health, Our Priority Since 1995")
  - Subheadline highlighting key specialties or USPs
  - Primary CTA (Book Appointment, Emergency Contact)
  - Hero image or video (professional, trust-building)
  - MUST be mobile-optimized and fast-loading

- **Emergency Contact Banner**:
  - Prominent emergency phone number
  - Ambulance service button/link
  - 24/7 availability indicator
  - Sticky or always-visible on scroll (optional but recommended)

- **Key Departments**:
  - Visual cards showcasing 6-8 primary departments
  - Icons and brief descriptions
  - Link to full department page

- **Featured Doctors**:
  - Carousel or grid of top 4-6 doctors
  - Photos, names, specialties, "Book Appointment" CTA
  - Link to full doctors listing

- **Appointment Booking CTA**:
  - Prominent section encouraging online appointment booking
  - Quick booking form or link to full booking page
  - Trust indicators (easy cancellation, confirmed slots)

- **Services Overview**:
  - Grid or list of key services offered
  - Icons and brief descriptions
  - Link to services page

- **Patient Testimonials**:
  - 3-5 patient success stories with photos (with consent)
  - Star ratings and review quotes
  - Video testimonials (optional)
  - MUST obtain patient consent for testimonials
  - MUST NOT include PHI in testimonials

- **Certifications & Accreditations**:
  - Logos of certifications (JCI, NABH, ISO, etc.)
  - Partner logos (insurance companies, medical associations)
  - Awards and recognitions

- **Health Information**:
  - Recent blog posts or health tips
  - Link to health library or blog

- **Facilities**:
  - Modern equipment and infrastructure highlights
  - Photo gallery or virtual tour link
...
- **Footer**:
  - Quick links (About, Services, Doctors, Departments, Contact, Careers)
  - Contact information
  - Social media links
  - Emergency contact (repeated)
  - Privacy policy, terms of service
  - Accessibility statement
  - Sitemap link

**Trust-Building & Credibility Requirements:**

- MUST display hospital certifications and accreditations prominently
- MUST display professional associations and memberships
- MUST display awards and recognitions
- MUST display real patient testimonials (with consent, no PHI)
- MUST display qualified doctor credentials prominently
- MUST use professional photography (high quality, trust-inspiring)
- MUST avoid stock photos that look generic or fake
- MUST use medical color psychology (blue for trust, green for health, white for cleanliness)
- MUST display years of operation or experience
- MUST showcase modern facilities and equipment
- MUST display emergency services availability (24/7 if applicable)
- MUST provide clear, transparent pricing information (if applicable)
- MUST display insurance partnerships
- MUST provide clear contact information and responsiveness indicators
- SHOULD display success rates or outcome statistics (if permitted and accurate)
- SHOULD display case studies or patient success stories (with consent)
- SHOULD provide virtual tour or facility photos
- SHOULD display community involvement and CSR activities

**SEO & Discoverability for Healthcare:**

Beyond standard SEO requirements, hospital websites MUST implement healthcare-specific optimizations:

- MUST implement Schema.org markup for healthcare:
  - Hospital schema (organization type)
  - MedicalOrganization schema with accreditations
  - Physician schema for doctor profiles
  - MedicalSpecialty schema for departments
  - MedicalProcedure schema for treatments/services
  - MedicalClinic schema for multi-location hospitals
- MUST optimize for local SEO:
  - Google My Business listing for each location
  - Local citations (healthcare directories, review sites)
  - Location-specific landing pages for each branch
  - NAP consistency (Name, Address, Phone) across all platforms
- MUST optimize for healthcare-specific keywords:
  - Specialty-based keywords (e.g., "cardiologist in [city]")
  - Treatment-based keywords (e.g., "knee replacement surgery [city]")
  - Symptom-based keywords (e.g., "chest pain doctor near me")
  - Emergency services keywords (e.g., "24/7 emergency hospital [city]")
- MUST implement health information content strategy:
  - Educational blog posts on common conditions
  - Preventive care guides
  - Health tips and wellness content
  - Disease awareness content
  - Treatment procedure explanations
- MUST optimize page titles and meta descriptions for patient search intent
- MUST implement FAQ schema for common patient questions
- MUST create location pages for multi-branch hospitals
- MUST implement review schema and encourage patient reviews (compliant with regulations)
- SHOULD create content for voice search queries (conversational, question-based)
- SHOULD implement video schema for patient testimonials and doctor introductions
- SHOULD create specialty landing pages optimized for paid search campaigns

**Accessibility Requirements (Enhanced for Healthcare):**

Healthcare websites serve diverse populations including elderly, disabled, and those with limited digital literacy.

- MUST meet WCAG 2.1 Level AA at minimum (ADA compliance)
- SHOULD meet WCAG 2.1 Level AAA for critical patient-facing features
- MUST support screen readers (JAWS, NVDA, VoiceOver)
- MUST provide alternative text for all medical images and diagrams
- MUST provide high contrast mode for visually impaired users
- MUST support text resizing without breaking layout (up to 200%)
- MUST implement clear, simple language (avoid medical jargon or provide glossary)
- MUST provide skip navigation links
- MUST ensure all interactive elements are keyboard accessible
- MUST provide clear focus indicators for all interactive elements
- MUST ensure minimum touch target size of 44x44px for mobile
- MUST provide clear error messages and guidance in forms
- MUST provide breadcrumbs and clear navigation
- MUST ensure readable font sizes (minimum 16px for body text)
- MUST provide captions for video content
- MUST provide transcripts for audio content
- SHOULD provide multilingual support for diverse patient populations
- SHOULD provide text-to-speech functionality for health information
- SHOULD provide simplified/easy-to-read version of complex medical information

**Multi-Language Support:**

Hospital websites MUST support internationalization for diverse patient populations:

- MUST support at minimum: English and local language(s)
- SHOULD support additional languages based on patient demographics
- MUST translate all critical patient-facing content:
  - Navigation and UI labels
  - Appointment booking forms
  - Emergency contact information
  - Service descriptions (at minimum, basic information)
  - Contact forms and patient communications
- SHOULD translate doctor profiles and department information
- SHOULD translate health information and blog content
- MUST implement language selector (prominent, usually top-right or navigation)
- MUST implement proper internationalization framework (i18n)
- MUST ensure SEO optimization for each language (hreflang tags)
- MUST ensure right-to-left (RTL) support if supporting Arabic, Hebrew, Urdu
- MUST ensure culturally appropriate imagery and content for each language
- SHOULD implement automatic language detection based on browser settings
- SHOULD provide language-specific contact information if applicable

**Performance Requirements (Enhanced for Healthcare):**

- MUST meet Core Web Vitals "Good" thresholds (LCP <2.5s, INP <200ms, CLS <0.1)
- MUST load initial page in <3 seconds on 3G connection
- MUST prioritize above-the-fold content (emergency contact, appointment CTA)
- MUST implement image optimization for doctor photos and facility images
- MUST implement lazy loading for images below the fold
- MUST minimize JavaScript for fast initial load
- MUST implement caching strategy for frequently accessed pages
- MUST implement CDN for static assets
- MUST optimize for mobile (majority of patient traffic is mobile)
- MUST ensure appointment booking form loads quickly (<2 seconds)
- SHOULD implement Progressive Web App (PWA) for offline access to key information
- SHOULD implement service worker for offline emergency contact information

**Security Requirements (Enhanced for Healthcare):**

Beyond standard security requirements, hospital websites MUST implement:

- MUST use HTTPS with HSTS headers (Strict-Transport-Security)
- MUST implement Content Security Policy (CSP) headers
- MUST implement secure password policies (minimum 12 characters for patient portal)
- MUST implement multi-factor authentication (MFA) for patient portal access
- MUST implement account lockout after failed login attempts
- MUST implement secure password reset process
- MUST implement session management with timeout (15 minutes for PHI access)
- MUST implement CSRF protection for all forms
- MUST sanitize all user inputs to prevent XSS and injection attacks
- MUST implement rate limiting for appointment forms and contact forms
- MUST implement spam protection (reCAPTCHA v3 or similar)
- MUST scan uploaded files for viruses (if file upload enabled)
- MUST encrypt all data transmission (TLS 1.3+)
- MUST implement secure API authentication for HMS integration
- MUST conduct regular security audits and penetration testing
- MUST implement Web Application Firewall (WAF) for production
- MUST implement DDoS protection
- MUST maintain security patch schedule for dependencies
- MUST implement security headers (X-Content-Type-Options, X-Frame-Options, Referrer-Policy)
- MUST implement logging and monitoring for security events
- MUST have incident response plan for security breaches
- SHOULD implement IP whitelisting for administrative access

**Mobile-First Design Requirements:**

Majority of patients access hospital websites via mobile devices:

- MUST design mobile-first (320px to 4K+ responsive)
- MUST ensure touch-friendly interface (minimum 44x44px touch targets)
- MUST optimize forms for mobile input (appropriate input types, minimal fields)
- MUST implement click-to-call for phone numbers
- MUST implement click-to-email for email addresses
- MUST implement one-tap directions to hospital location
- MUST ensure appointment booking works seamlessly on mobile
- MUST ensure fast mobile load times (<3 seconds on 3G)
- MUST test on iOS and Android devices
- MUST ensure hamburger menu is accessible and intuitive
- MUST ensure emergency contact is visible on mobile (sticky header or prominent placement)
- SHOULD implement app download links if native app available
- SHOULD implement mobile-specific features (geolocation for nearest branch)

**Communication & Notifications:**

- Email notifications for appointment confirmations (MUST)
- SMS notifications for appointment confirmations (MUST)
- Email appointment reminders (24 hours before) (SHOULD)
- SMS appointment reminders (2 hours before) (SHOULD)
- WhatsApp notifications (optional, if supported in region)
- Push notifications via PWA (optional)
- Newsletter subscription for health tips and hospital updates (optional)
- MUST implement notification preferences (email, SMS, both, none)
- MUST respect opt-out requests
- MUST provide unsubscribe mechanism for marketing communications
- MUST comply with communication regulations (CAN-SPAM, GDPR consent)

**Integration Requirements:**

- **HMS Integration**: API for appointments, doctors, patient portal (MUST if patient portal enabled)
- **Payment Gateway**: Stripe, Razorpay, PayPal for online payments (SHOULD if accepting payments)
- **SMS Gateway**: Twilio, SNS, or regional SMS provider for appointment confirmations (MUST)
- **Email Service**: SendGrid, AWS SES, Mailgun for transactional emails (MUST)
- **WhatsApp API**: For appointment confirmations and reminders (optional)
- **Google Maps**: For location and directions (MUST)
- **Analytics**: Google Analytics, Plausible, or privacy-respecting alternative (MUST)
- **CRM**: HubSpot, Salesforce for lead management (optional)
- **Live Chat**: Intercom, Drift, Tawk.to for patient inquiries (optional)
- **Review Platforms**: Google Reviews, Healthgrades, Practo integration (SHOULD)

**AI-Powered Enhancements for Hospital Websites:**

- AI chatbot for answering common patient questions (FAQs, services, appointments)
- Intelligent appointment booking assistant (helps patients find right doctor/department)
- Symptom checker (basic, with disclaimer - "not a substitute for professional medical advice")
- AI-powered search for doctors, services, health information
- Personalized health content recommendations based on browsing behavior
- Automated appointment reminders with natural language SMS/email
- AI-assisted live chat routing (categorize inquiries and route to appropriate department)
- Natural language processing for patient feedback sentiment analysis
- Automated language translation for multilingual support
- Voice search and voice-based appointment booking (accessibility feature)
- AI-powered content generation for health blogs and FAQs (with medical professional review)
- Predictive analytics for appointment booking trends and resource planning
- AI-based image recognition for accessibility (describe medical images for visually impaired)

**IMPORTANT AI Safety for Healthcare:**

- AI chatbots MUST include disclaimer: "This information is for general guidance only and is not a substitute for professional medical advice. Please consult a qualified healthcare provider for diagnosis and treatment."
- AI symptom checkers MUST NOT provide diagnosis or treatment recommendations
- AI-generated content MUST be reviewed and approved by qualified medical professionals before publication
- AI MUST NOT provide medical advice, prescriptions, or treatment plans
- AI MUST escalate to human support for serious medical inquiries
- AI MUST comply with medical advertising regulations (no false claims)
- AI MUST NOT collect or process PHI without proper consent and HIPAA compliance

**Testing Requirements (Enhanced for Healthcare):**

- MUST test appointment booking flow end-to-end (user journey testing)
- MUST test SMS and email notifications delivery and content
- MUST test HMS integration (if applicable)
- MUST test payment gateway integration (if applicable)
- MUST test form validation and error handling
- MUST test cross-browser compatibility (Chrome, Safari, Firefox, Edge)
- MUST test mobile responsiveness on iOS and Android devices
- MUST test accessibility with screen readers (NVDA, VoiceOver)
- MUST test performance on 3G connection (mobile network simulation)
- MUST test security (penetration testing, vulnerability scanning)
- MUST test multi-language functionality (if applicable)
- MUST test emergency contact visibility and click-to-call
- MUST load test appointment booking system for peak traffic
- SHOULD test with real patients (user testing for usability)
- SHOULD test email deliverability (spam filter testing)

**Launch Checklist for Hospital Websites:**

- [ ] Hospital configuration file tested and verified (branding, content, integrations)
- [ ] HIPAA compliance reviewed (if collecting PHI)
- [ ] GDPR compliance verified (if serving EU residents)
- [ ] Local healthcare regulations compliance verified
- [ ] Privacy policy and Notice of Privacy Practices published
- [ ] Terms of service published
- [ ] Data encryption at rest and in transit verified
- [ ] HTTPS with HSTS configured and tested
- [ ] Security headers configured (CSP, X-Content-Type-Options, etc.)
- [ ] Authentication and authorization tested (patient portal if enabled)
- [ ] Appointment booking flow tested end-to-end
- [ ] SMS notification delivery tested
- [ ] Email notification delivery tested
- [ ] HMS integration tested (if applicable)
- [ ] Payment gateway tested (if applicable)
- [ ] Google Maps integration tested
- [ ] Doctor profiles accuracy verified
- [ ] Department and service information accuracy verified
- [ ] Emergency contact information verified and prominent
...
- [ ] Multi-language testing completed (if applicable)
- [ ] SEO optimization verified (schema markup, meta tags, sitemap)
- [ ] Google My Business listing created and verified
- [ ] Analytics and monitoring configured
- [ ] Security audit and penetration testing completed
- [ ] Backup and disaster recovery tested
- [ ] SSL certificate installed and auto-renewal configured
- [ ] Content reviewed by medical professionals for accuracy
- [ ] Legal review of disclaimers and patient-facing content
- [ ] Staff training on appointment management completed
- [ ] Patient onboarding documentation prepared
- [ ] Incident response procedures documented
- [ ] HIPAA/HITECH training completed for relevant staff
- [ ] Business Associate Agreements (BAA) signed with third-party services

**Recommended Tech Stack for Hospital Websites:**

**Option 1: Next.js Full-Stack (Recommended for Most Hospital Websites)**

- **Frontend**: Next.js 14+ with App Router, TypeScript, Tailwind CSS
- **Backend**: Next.js API routes or tRPC for type-safe APIs
- **Database**: PostgreSQL (recommended) or MySQL
- **ORM**: Prisma or Drizzle ORM with TypeScript
- **Authentication**: NextAuth.js (for patient portal) or Clerk
- **CMS**: Sanity, Contentful, or Strapi for content management (doctors, departments, blog)
- **File Storage**: AWS S3, Cloudflare R2, or Vercel Blob for images
- **Email**: SendGrid, AWS SES, or Resend
- **SMS**: Twilio, AWS SNS, or regional SMS gateway
- **Maps**: Google Maps JavaScript API
- **Analytics**: Google Analytics, Plausible, or Vercel Analytics
- **Payments**: Stripe or Razorpay (if needed)
- **Testing**: Vitest + Testing Library + Playwright
- **Hosting**: Vercel (easiest), AWS, or Cloudflare Pages
- **Pros**: Excellent developer experience, built-in optimizations, great SEO, easy deployment
- **Cons**: Vercel vendor lock-in, serverless cold starts for patient portal

**Option 2: Python Django Full-Stack (Traditional, Healthcare-Industry Proven)**

- **Backend**: Django 5+ with Python 3.11+
- **Frontend**: React with TypeScript or Django Templates with Alpine.js/HTMX
- **Database**: PostgreSQL (recommended for healthcare compliance)
- **Authentication**: Django-allauth or django-authentication
- **CMS**: Django CMS, Wagtail, or Django admin for content
- **File Storage**: django-storages with S3
- **Email**: Django email backends (SendGrid, AWS SES)
- **SMS**: Twilio Python SDK, AWS SNS
- **Testing**: pytest + pytest-django + Playwright
- **Hosting**: AWS, Heroku, DigitalOcean, or Render
- **Pros**: Battle-tested in healthcare industry, robust admin interface, strong security defaults, mature ecosystem, better for complex HMS integration, easier HIPAA compliance audits
- **Cons**: Monolithic, less modern frontend patterns without React, steeper learning curve

**Option 3: Microservices with Next.js + FastAPI (Scalable, AI-Ready)**

- **Frontend**: Next.js 14+ with TypeScript
- **Backend**: Python FastAPI for APIs, HMS integration, AI features
- **Database**: PostgreSQL with row-level security
- **Authentication**: Auth0, Supabase Auth, or custom JWT with FastAPI
- **File Storage**: AWS S3 with encryption
- **Email/SMS**: SendGrid, Twilio via FastAPI services
- **AI/ML**: FastAPI microservices with OpenAI SDK, LangChain for chatbot
- **Testing**: pytest + Vitest + Playwright
- **Hosting**: Vercel (frontend) + AWS ECS/Lambda (backend)
- **Pros**: Excellent for AI chatbot features, scalable, modern, type-safe, flexible
- **Cons**: More complex deployment, requires managing multiple services, higher DevOps overhead

**Database Schema Design for Hospital Website:**

**Configuration-Based (Recommended):**

If using configuration files for multi-hospital deployment, schema is simpler:

```sql
-- Core tables (single hospital deployment)
doctors (
  id, name, designation, qualification, specialties,
  experience_years, photo_url, opd_schedule, consultation_fee,
...
services (
  id, name, description, department_id, created_at, updated_at
)
...
testimonials (
  id, patient_name, patient_photo_url, content, rating,
  approved, created_at, updated_at
)
```

**Multi-Tenant (For Platform Providers):**

If building a SaaS platform for multiple hospitals:

```sql
-- Add hospital_id to all tables
hospitals (
  id, name, slug, domain, branding_config, contact_info,
  features_config, created_at, updated_at
)

doctors (
  id, hospital_id, name, designation, qualification, ...
)
...
appointments (
  id, hospital_id, patient_name, doctor_id, ...
)

-- Implement row-level security (RLS) in PostgreSQL
-- Ensure all queries filter by hospital_id
```

**Key Design Considerations:**

- MUST implement proper foreign key constraints (ON DELETE CASCADE/RESTRICT)
- MUST implement indexes on frequently queried fields (doctor_id, department_id, appointment_date)
- MUST implement soft deletes for audit trail (deleted_at timestamp)
- MUST implement created_at and updated_at timestamps on all tables
- MUST implement database-level validation for data integrity
- MUST encrypt sensitive fields (patient contact information if storing beyond appointment booking)
- MUST implement connection pooling (PgBouncer) for production
- SHOULD implement read replicas for reporting queries if high traffic
- SHOULD implement full-text search indexes for doctor and service search

**Rationale**: Hospital websites require specialized features for patient acquisition, trust-building, and healthcare service promotion while ensuring strict compliance with healthcare privacy regulations (HIPAA, HITECH, GDPR). The configurable multi-hospital architecture enables efficient deployment for multiple healthcare institutions from a single codebase. Enhanced requirements ensure the platform meets the unique needs of healthcare providers while maintaining the highest standards for data privacy, security, accessibility, and patient experience. The clear distinction between public website (minimal compliance) and patient portal (full HIPAA compliance) provides a pragmatic approach to healthcare web development.
